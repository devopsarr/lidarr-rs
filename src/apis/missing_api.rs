...
pub async fn get_wanted_missing(configuration: &configuration::Configuration, page: Option<i32>, page_size: Option<i32>, sort_key: Option<&str>, sort_direction: Option<models::SortDirection>, include_artist: Option<bool>, monitored: Option<bool>) -> Result<models::AlbumResourcePagingResource, Error<GetWantedMissingError>> {
    ...
    let uri_str = format!("{}/api/v1/wanted/missing", configuration.base_path);
...
pub async fn get_wanted_missing_by_id(configuration: &configuration::Configuration, id: i32) -> Result<models::AlbumResource, Error<GetWantedMissingByIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;

    let uri_str = format!("{}/api/v1/wanted/missing/{id}", configuration.base_path, id=p_path_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
