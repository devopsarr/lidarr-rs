...
/// struct for typed errors of method [`list_artist_lookup`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListArtistLookupError {
    UnknownValue(serde_json::Value),
}


pub async fn list_artist_lookup(configuration: &configuration::Configuration, term: Option<&str>) -> Result<Vec<models::ArtistResource>, Error<ListArtistLookupError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_term = term;

    let uri_str = format!("{}/api/v1/artist/lookup", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListArtistLookupError> = serde_json::from_str(&content).ok();
